    ...
    try:
        ...
        if not os.path.exists(pdf_directory):
            ...
            raise HTTPException(
                ...
                detail=f"PDF directory not found: {pdf_directory}. Please place your PDF files in this directory."
            )
        ...
        if not pdf_files:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"No PDF files found in directory: {pdf_directory}"
            )
                ...
                for field_name, field_value in basic_fields:
                    if field_value:
                        data = ExtractedData(
                            ...
                            confidence_score=0.95,  # High confidence for AI extraction
...
async def get_batch_analysis_data(
    ...
    try:
        ...
        documents = db.query(COADocument).filter(
            ...
            COADocument.processing_status == ProcessingStatus.COMPLETED
        ).all()
